...
##### Pitää miettiä
PHY_bac_genus <- tax_glom(PHY_bac, taxrank = "genus")
PHY_bac_genus_Bacteroidota <- subset_taxa(PHY_bac_genus, phylum == "Bacteroidota")

#PHY_bac_genus_Bacteroidota_rel <- transform_sample_counts(PHY_bac_genus_Bacteroidota, function(x) x / sum(x))

# Reorder sample_type with custom levels
sample_data(PHY_bac_genus_Bacteroidota)$sample_type <- factor(sample_data(PHY_bac_genus_Bacteroidota)$sample_type,
                                       levels = c("influent", "effluent", "sludge"))
...
p <- plot_bar_2(PHY_bac_genus_Bacteroidota, fill = "class") + 
  geom_bar(stat = "identity") + theme_minimal() + 
  ggtitle("Bacteroidota composition, WW data (Sylph)") +
  scale_y_continuous(expand = c(0,0), limits = c(0,1)) +
  theme(
        text = element_text(family = "Palatino"),
        legend.text = element_text(face = "italic", size = 20),
...
# Diversity of Bacteroidia across samples
```{r}
PHY_bac_genus <- tax_glom(PHY_bac, taxrank = "genus")
PHY_bac_genus_Bacteroidota <- subset_taxa(PHY_bac_genus, phylum == "Bacteroidota")
PHY_bac_genus_Bacteroidota <- prune_taxa(taxa_sums(PHY_bac_genus_Bacteroidota) > 0, PHY_bac_genus_Bacteroidota)
...
div <- plot_richness(PHY_bac_genus_Bacteroidota, x = sample)
```
