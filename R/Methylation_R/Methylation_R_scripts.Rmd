...
WW_kaiju_summary_filt$taxon_name <- factor(WW_kaiju_summary_filt$taxon_name, 
                                        levels=c("Methylotenera", "Rhodoferax", "Arcobacter", "Acinetobacter", "Flavobacterium", "Aliarcobacter", "Acidovorax", "Lactococcus", "Aeromonas", "Bacteroides", "Candidatus Cloacimonas", "Anaerolinea", "Methanothrix", "Petrimonas", "Brevefilum", "Pelolinea", "Viruses", "cannot be assigned to a (non-viral) genus", "unclassified"))
  
p <- ggplot(WW_kaiju_summary_filt, aes(x = sample, y = percent, fill = taxon_name)) + 
  geom_bar(stat = "identity") + theme_minimal() + 
  ggtitle("Bacterial composition, WW dataset (Kaiju)") +
  scale_y_continuous(expand = c(0,0), limits = c(0,100)) +
  theme(
        text = element_text(family = "Palatino"),
        legend.text = element_text(face = "italic", size = 30),
...
# Try those > 10 % 
```{r}
cols <- c("#34495E","#87F28A", "#F1C40F", "#080bf0", "#605c5a", "#1ABC9C","#E74C3C", 
          "#FFB6B0", "#c9f063",  "#8C510A", "#808000", "#00FFFF", "#FF00FF", "#E67E22", "#16A085",
          "#2980B9", "#D35400", "#f6b178", "#7304b6", "#A3E4D7", "#C0392B", "#04b627", "#AF7AC5", "black")
...
p <- plot_bar(PHY_bac_genus_rel_filtered, fill = "genus") + 
  geom_bar(stat = "identity") + theme_minimal() + 
  ggtitle("Most abundant genera (>10 %), WW dataset (Sylph)") +
